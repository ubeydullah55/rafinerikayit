    ...
    <div class="pd-ltr-20 xs-pd-20-10">
        <div class="min-height-200px">
            <div class="page-header">
                <!-- Seçilebilir veya yazılabilir input -->
                <form action="<?= base_url('musteriIslem/musteriBilgiGetir') ?>" method="post" class="ml-auto d-flex align-items-center gap-2">
                    <?= csrf_field() ?> <!-- CSRF koruması için CodeIgniter -->

                    <label for="musteriSelect" class="mb-0">Müşteri Listesi:</label>
                    <select id="musteriSelect" name="musteri_id" class="form-control" style="width: 250px;">
                        <option value="" selected>Bir müşteri seçin...</option>
                        <?php foreach ($musteriler as $musteri): ?>
                            <option value="<?= esc($musteri['id']) ?>"><?= esc($musteri['ad']) ?></option>
                        <?php endforeach; ?>
                    </select>

                    <button type="submit" class="btn btn-primary ml-3">Bilgileri Getir</button>
                </form>
            ...
            <div class="card-box mb-30">
                <div class="pd-20">
                    <?php if (!empty($musteriBilgi['ad'])): ?>
                        <h4 class="text-blue h4"><?= esc($musteriBilgi['ad']); ?></h4>
                        ...
                        <div class="mt-2">
                            <label for="musteriOran">Oran:</label>
                            <input type="text" id="musteriOran" class="form-control"
                                value="<?= isset($musteriBilgi['oran']) && $musteriBilgi['oran'] !== '' ? esc($musteriBilgi['oran']) : '0'; ?>" readonly>
                        </div>
                    <?php endif; ?>
                </div>
                <div class="pb-20">
                    ...
                    <table class="table hover multiple-select-row data-table-export nowrap">
                        ...
                        <tbody>
                            <?php if (!empty($takozlar)): ?>
                                <?php foreach ($takozlar as $item): ?>
                                    <?php
                                    ...
                                    } elseif (isset($item['status_code']) && $item['status_code'] == 8) {
                                        // Hafif mavi arkaplan
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                                    ...
                                                    <?php if ((int)$item['status_code'] === 7): ?>
                                                        <a class="dropdown-item" href="#"
                                                            onclick="onaylaTakoz(<?= $item['id']; ?>)">
...
<script>
    function onaylaTakoz(takoz_id) {
        $.ajax({
            url: "<?= base_url('musteriIslem/onaylaTakoz') ?>",
            method: "POST",
            data: {
                takoz_id: takoz_id
            },
